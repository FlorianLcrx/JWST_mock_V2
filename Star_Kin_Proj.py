...
def process_subfind_id(indices, basepath, snapnum, hubble_param, Subhalo_pos, Subhalo_Velocity, scaling_factor, R_star):
    ...
    Subhalo_stars_part = il.snapshot.loadSubhalo(basepath, snapNum=snapnum, id=indices, partType='stars', fields=["GFM_StellarFormationTime","Masses","Coordinates","Velocities"])
    #Subhalo_gas_part = il.snapshot.loadSubhalo(basepath, snapNum=snapnum, id=indices, partType='gas', fields=["StarFormationRate","Masses","Coordinates"])
    ...
    Coord_stars = (Subhalo_stars_part["Coordinates"][Condition_to_remove_wind]-Subhalo_pos[indices]) * scaling_factor / hubble_param
    velocity_stars = Subhalo_stars_part["Velocities"][Condition_to_remove_wind]*np.sqrt(scaling_factor)
    
    a, b, c, epsilon, Ttriaxial, eigenvectors = Defining_new_coords(Coord_stars, Mass_stars, r_star)
    if not np.isnan(a):
        Part_velocity_along_b ,Part_velocity_along_c,mean_velocities,mean_coords,V_max,V_max_comp,V_disp,std_lof,mean_coords_std =  Kinematics_of_the_galaxie(Coord_stars,velocity_stars, Subhalo_Velocity[indices], eigenvectors, r_star, scaling_factor)
    else : 
        Part_velocity_along_b ,Part_velocity_along_c,mean_velocities,mean_coords,V_max,V_max_comp,V_disp,std_lof,mean_coords_std =  Kinematics_of_the_galaxie(Coord_stars,velocity_stars, Subhalo_Velocity[indices], np.eye(3), r_star, scaling_factor)
...
Snap_nums = [99,50] #z = 0,1,2,3,4,5,6
...
for i in range(len(Snap_nums)):
    ...
    with h5py.File(f'New_result_stars_with_kinematics_slit25{snapnum}', 'w') as hf:  # Use 'a' for append mode
